...
void sustained_load(uint32_t type) {
    while (!finish_load.load(std::memory_order_relaxed)) {
        ba_slice_t slice;
        TEST_EQUAL(ba_claim(type, 1024, 0, &slice), BA_OK, "sustained claim failed");
        ba_release(&slice);
    }
}
/** --------------------------------------------------------------------------------------------------------- Slab Edges
...
void slab_edges(uint32_t type) {
    ...
    for (unsigned iteration = 0; iteration < 16; ++iteration) {
        ...
        TEST_EQUAL(ba_claim(type, bytes, 0, &slice), BA_OK, "edge claim failed");
        auto* memory = static_cast<unsigned char*>(slice.ptr);
        for (size_t index = 0; index < 4096; ++index) {
            TEST_REQUIRE(memory[index] == 0 && memory[bytes - 4096 + index] == 0, "recycled edge dirty");
        }
...
void rollovers(uint32_t type) {
    for (size_t index = 0; index < 100000; ++index) {
        ba_slice_t slice;
        TEST_EQUAL(ba_claim(type, 1024, 0, &slice), BA_OK, "rollover claim failed");
        auto* memory = static_cast<unsigned char*>(slice.ptr);
        TEST_REQUIRE(memory[0] == 0 && memory[1023] == 0, "dirty claim");
...
int main() {
    ...
    uint32_t type;
    TEST_EQUAL(ba_placement_register(&descriptor, &type), BA_OK, "registration failed");
    TEST_REQUIRE(background_allocations.load() >= 2, "initial slabs were not built by worker");
    ...
    TEST_REQUIRE(statistics.slab_bytes_allocated >= 6 * statistics.slab_bytes, "insufficient rollovers");
    TEST_REQUIRE(statistics.slab_bytes_allocated - statistics.slab_bytes_freed <= (statistics.runway_target + 1) * statistics.slab_bytes, "retired slabs retained");
    ...
    TEST_REQUIRE(statistics.runway_misses <= system.hw_threads, "excessive runway misses");
    std::this_thread::sleep_for(std::chrono::seconds(1));
    ba_stats(type, &statistics);
    TEST_REQUIRE(statistics.reserved_bytes <= (statistics.runway_target + 1) * statistics.slab_bytes, "reserve cap exceeded");
    ba_slice_t novel;
    TEST_EQUAL(ba_claim(type, 8ull * 1024 * 1024, BA_CLAIM_NOVEL, &novel), BA_OK, "novel failed");
    ...
    TEST_REQUIRE(background_frees.load() > freed, "novel not freed by worker");
    ba_trim(type);
    ba_stats(type, &statistics);
    TEST_EQUAL(statistics.reserved_bytes, 0, "trim left reserves");
    ...
    uint32_t cache_type;
    TEST_EQUAL(ba_placement_register(&descriptor, &cache_type), BA_OK, "cache registration failed");
    TEST_EQUAL(ba_claim(cache_type, 8ull * 1024 * 1024, BA_CLAIM_NOVEL, &novel), BA_OK, "cache first claim failed");
    ...
    TEST_EQUAL(statistics.novel_cache_bytes, 8ull * 1024 * 1024, "novel did not enter cache");
    TEST_EQUAL(ba_claim(cache_type, 8ull * 1024 * 1024, BA_CLAIM_NOVEL, &novel), BA_OK, "cache second claim failed");
    TEST_EQUAL(cached_novel_allocations.load(), 1, "cache missed a prepared allocation");
    for (size_t index = 0; index < 8ull * 1024 * 1024; ++index) {
        TEST_EQUAL(static_cast<unsigned char*>(novel.ptr)[index], 0, "cached novel was not zeroed");
    }
    ba_release(&novel);
    ba_trim(cache_type);
    ba_stats(cache_type, &statistics);
    TEST_REQUIRE(statistics.reserved_bytes == 0 && statistics.novel_cache_bytes == 0, "trim retained cached allocations");
    ...
    TEST_EQUAL(ba_placement_register(&descriptor, &budget_type), BA_OK, "budget registration failed");
    ...
    ba_slice_t held[3];
    TEST_EQUAL(ba_claim(budget_type, descriptor.slab_bytes, BA_CLAIM_NOVEL, &held[0]), BA_OK, "first budgeted allocation failed");
    TEST_EQUAL(ba_claim(budget_type, descriptor.slab_bytes, BA_CLAIM_NOVEL, &held[1]), BA_OK, "second budgeted allocation failed");
    TEST_EQUAL(ba_claim(budget_type, descriptor.slab_bytes, BA_CLAIM_NOVEL, &held[2]), BA_E_BUDGET, "third allocation exceeded budget");
    TEST_EQUAL(ba_claim(budget_type, descriptor.slab_bytes / 2, 0, &novel), BA_OK, "budget denied an existing-slab claim");
    ba_release(&held[0]);
    ...
    TEST_EQUAL(ba_claim(budget_type, descriptor.slab_bytes, BA_CLAIM_NOVEL, &held[2]), BA_OK, "released capacity was not reusable");
    ba_release(&held[1]); ba_release(&held[2]); ba_release(&novel);
    TEST_REQUIRE(ba_pressure() <= BA_PRESSURE_CRITICAL, "invalid pressure result");
    const uint64_t stop_begin = ba_os_now_ns();
    ba_shutdown();
    TEST_REQUIRE(ba_os_now_ns() - stop_begin < 10000000000ull, "shutdown timed out");
    TEST_EQUAL(ba_claim(type, 64, 0, &novel), BA_E_CLOSED, "claim after shutdown succeeded");
}
