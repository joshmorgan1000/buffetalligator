...
void sizes(uint32_t type) {
    for (size_t size : {1, 63, 64, 65, 1000, 4096, 65536}) {
        for (size_t operation = 0; operation < 10000; ++operation) {
            ba_slice_t slice;
            TEST_EQUAL(ba_claim(type, size, 0, &slice), BA_OK, "claim failed");
            TEST_EQUAL(reinterpret_cast<uintptr_t>(slice.ptr) % 64, 0, "unaligned claim");
            TEST_EQUAL(slice.meta >> BA_SLOT_BITS, size, "incorrect size");
            const auto* begin = static_cast<unsigned char*>(slice.ptr);
            TEST_REQUIRE(std::all_of(begin, begin + size, [](unsigned char value) { return value == 0; }), "claim not zeroed");
            std::memset(slice.ptr, 0xa5, size);
            ba_release(&slice);
        }
    }
    ba_slice_t first, second;
    TEST_EQUAL(ba_claim(type, 4096, 0, &first), BA_OK, "first adjacent claim failed");
    TEST_EQUAL(ba_claim(type, 1024, 0, &second), BA_OK, "second adjacent claim failed");
    TEST_EQUAL(static_cast<char*>(second.ptr) - static_cast<char*>(first.ptr), 4096, "nonadjacent claims");
    ba_release(&first); ba_release(&second);
    ba_stats_t statistics;
    ba_stats(type, &statistics);
    ba_slice_t before, direct, after;
    TEST_EQUAL(ba_claim(type, 64, 0, &before), BA_OK, "before direct failed");
    TEST_EQUAL(ba_claim(type, statistics.plate_bytes + 64, 0, &direct), BA_OK, "direct failed");
    TEST_EQUAL(ba_claim(type, 64, 0, &after), BA_OK, "after direct failed");
    TEST_REQUIRE((direct.meta & BA_SLOT_MASK) != (before.meta & BA_SLOT_MASK), "direct reused thread plate");
    TEST_REQUIRE(!ba_slice_is_novel(&before) && !ba_slice_is_novel(&direct), "a slab plate reported novel backing");
    TEST_EQUAL(static_cast<char*>(after.ptr) - static_cast<char*>(before.ptr), 64, "direct changed thread cursor");
    ba_slice_t whole;
    TEST_EQUAL(ba_claim(type, statistics.slab_bytes, 0, &whole), BA_OK, "slab-sized claim failed");
    TEST_REQUIRE(ba_slice_handle(&whole) != ba_slice_handle(&after), "slab-sized claim reused slab handle");
    TEST_REQUIRE(!ba_slice_is_novel(&whole), "a full slab reported novel backing");
    ba_release(&before); ba_release(&direct); ba_release(&after); ba_release(&whole);
    if (type == 2) {
        ba_slice_t oversized;
        TEST_EQUAL(ba_claim(type, statistics.slab_bytes + 64, 0, &oversized), BA_OK, "oversized claim failed");
        TEST_REQUIRE(ba_slice_is_novel(&oversized), "automatic novel routing was not reported");
        ba_release(&oversized);
        TEST_REQUIRE(!ba_slice_is_novel(&oversized), "released core Slice reported novel backing");
    }
}
/** --------------------------------------------------------------------------------------------------------- Concurrent Claims
...
void concurrent(uint32_t type) {
    for (size_t operation = 0; operation < 100000; ++operation) {
        ba_slice_t slice;
        TEST_EQUAL(ba_claim(type, 1024, 0, &slice), BA_OK, "concurrent claim failed");
        auto* memory = static_cast<unsigned char*>(slice.ptr);
        TEST_REQUIRE(memory[0] == 0 && memory[1023] == 0, "concurrent dirty claim");
...
void held_slab(uint32_t type, ba_slice_t* held) {
    ...
    TEST_EQUAL(ba_claim(type, statistics.slab_bytes / 2, 0, held), BA_OK, "held direct failed");
    ba_retain(held);
    ba_slice_t copy = *held;
    ba_release(&copy);
    for (unsigned index = 0; index < 4; ++index) {
        ba_slice_t slice;
        TEST_EQUAL(ba_claim(type, statistics.slab_bytes / 2, 0, &slice), BA_OK, "held rollover failed");
        ba_release(&slice);
    }
}
}
...
int main() {
    ...
    TEST_REQUIRE(ba_placement_count() == 2 && ba_placement_default() == 1, "builtin identifiers");
    TEST_REQUIRE(!std::strcmp(ba_placement_name(0), "heap") && !std::strcmp(ba_placement_name(1), "aligned_heap"), "builtin names");
    ...
    uint32_t type;
    TEST_REQUIRE(ba_placement_register(&descriptor, &type) == BA_OK && type == 2, "custom registration");
    ...
    TEST_REQUIRE(before.slab_bytes_allocated - before.slab_bytes_freed >= 2 * before.slab_bytes, "held slab retired early");
    ba_release(&held);
    ...
    TEST_REQUIRE(after.slab_bytes_freed + after.reserved_bytes >= before.slab_bytes_freed + before.reserved_bytes + after.slab_bytes, "held slab not retired");
    ...
    TEST_REQUIRE(after.slab_bytes_allocated - after.slab_bytes_freed <= (after.runway_target + 1) * after.slab_bytes, "slabs leaked after thread exit");
    descriptor.base_alignment = 16;
    TEST_EQUAL(ba_placement_register(&descriptor, &type), BA_E_ALIGNMENT, "bad alignment accepted");
    ...
    TEST_EQUAL(ba_placement_register(&descriptor, &type), BA_E_ALLOC, "failed host accepted");
    TEST_REQUIRE(failed_substrates.load() == 1 && failed_handles.load() == 1, "failed host handle leaked");
    ba_slice_t invalid;
    TEST_EQUAL(ba_claim(ba_placement_count(), 64, 0, &invalid), BA_E_PLACEMENT, "unknown placement accepted");
}
