...
/** --------------------------------------------------------------------------------------------------------- Require
 * @brief Stops the test immediately when a registry invariant fails.
 */
...
int main() {
    ...
    require(ba_placement_register(&description, &placement) == BA_OK, "placement registration failed");
    ...
    require(initial > stride && initial < BA_PLATE_COUNT, "registry did not start small");
    ...
    for (size_t index = 0; index < prefix_claims; ++index) {
        require(ba_claim(placement, 64, BA_CLAIM_NOVEL, &held[index]) == BA_OK,
            "initial claim failed");
    }
    require(ba_slot_capacity() == initial, "initial claims grew the registry early");
    *static_cast<uint64_t*>(held[0].ptr) = 0x123456789abcdef0ull;
    ba_slice_t anchor;
    require(ba_view(&held[0], 0, 16, &anchor) == BA_OK, "anchor view failed");
    ...
    for (size_t attempt = 0; attempt < 4; ++attempt) {
        ...
        require(ba_claim(placement, 64, BA_CLAIM_NOVEL, &rejected) == BA_E_ALLOC,
            "commit failure did not report BA_E_ALLOC");
        require(rejected.meta == BA_NULL_META && rejected.ptr == nullptr,
            "commit failure published a slice");
    }
    ba_slice_t rejected_plate{BA_NULL_META, nullptr};
    require(ba_claim(placement, 64, 0, &rejected_plate) == BA_E_ALLOC,
    ...
    require(before.novel_bytes_allocated == after.novel_bytes_allocated,
        "commit failure allocated backing");
    require(ba_slot_capacity() == initial, "commit failure published capacity");
    ...
    require(ba_claim(placement, 64, BA_CLAIM_NOVEL, &held[prefix_claims]) == BA_OK,
        "claim after failed growth did not recover");
    ba_budget_set(placement, description.budget_bytes);
    require((held[prefix_claims].meta & BA_SLOT_MASK) == initial,
        "failed growth consumed a fresh slot");
    require(ba_slot_capacity() > initial, "successful commit did not grow capacity");
    ...
    std::thread reader([&] {
        ...
        do {
            ba_slice_t view;
            require(ba_view(&anchor, 0, 8, &view) == BA_OK, "view during growth failed");
            require(view.ptr == original_pointer &&
    ...
    for (size_t worker = 0; worker < threads; ++worker) {
        workers.emplace_back([&, worker] {
            ...
            for (size_t index = first; index < last; ++index) {
                ...
                require(*static_cast<uint64_t*>(held[index].ptr) == 0, "backing is not zeroed");
    ...
    reader.join();
    require(views.load() != 0, "concurrent reader did not execute");
    require(ba_slot_capacity() == BA_PLATE_COUNT, "registry did not reach its encoding ceiling");
    ...
    for (size_t index = 0; index < held.size(); ++index) {
        ...
        require(slice.meta >> BA_SLOT_BITS == 64, "growth changed the size encoding");
        if (index >= begin) require(*static_cast<uint64_t*>(slice.ptr) == index,
            "concurrent backing allocations overlap");
    }
    std::sort(slots.begin(), slots.end());
    for (size_t index = 0; index < slots.size(); ++index)
        require(slots[index] == index + 1, "slot IDs are duplicated or missing");
    for (size_t attempt = 0; attempt < 1024; ++attempt) {
        ba_slice_t rejected{BA_NULL_META, nullptr};
        require(ba_claim(placement, 64, BA_CLAIM_NOVEL, &rejected) == BA_E_SLOTS,
            "exhaustion did not report BA_E_SLOTS");
    }
    const uint64_t reusable_slot = held.back().meta & BA_SLOT_MASK;
    ba_release(&held.back());
    ba_trim(placement);
    require(ba_claim(placement, 64, BA_CLAIM_NOVEL, &held.back()) == BA_OK,
        "exhausted registry could not reuse a returned slot");
    require((held.back().meta & BA_SLOT_MASK) == reusable_slot, "returned slot was not reused");
    for (auto& slice : held) ba_release(&slice);
    ba_trim(placement);
    ba_stats(placement, &after);
    require(after.novel_bytes_allocated - after.novel_bytes_freed == 64,
        "failure or exhaustion leaked backing charges");
    ba_shutdown();
    require(anchor.ptr == original_pointer &&
        *static_cast<uint64_t*>(anchor.ptr) == 0x123456789abcdef0ull,
        "shutdown invalidated live registry storage");
    ba_release(&anchor);
    ba_stats(placement, &after);
    require(after.novel_bytes_allocated == after.novel_bytes_freed,
