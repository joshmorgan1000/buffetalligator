...
/**
 * @class SwiftUdpBuffer
 * @brief UDP network buffer using cswift's Network.framework bindings
 * 
 * This class provides a UDP network buffer that leverages cswift's
 ...
 */
class SwiftUdpBuffer : public NICBuffer {
private:
    ...
    struct RxData {
        size_t offset;
        size_t size;
        NetworkEndpoint sender;  // UDP needs to track sender for each packet
    };
...
public:
     ...
     */
    SwiftUdpBuffer(size_t capacity);

    /**
     * @brief Destructor
     */
    ~SwiftUdpBuffer() override;

    /**
     * @brief Factory method to create a SwiftUdpBuffer
     * @param capacity Size in bytes
     * @return Pointer to the created SwiftUdpBuffer
     */
    static SwiftUdpBuffer* create(size_t capacity) {
        return new SwiftUdpBuffer(capacity);
    }
    ...
    /**
     * @brief Connect to a remote endpoint (for connected UDP)
    ...
    /**
     * @brief Send datagram to specific endpoint
     * @param data Data to send
     * @param size Size of data
     * @param endpoint Target endpoint
     * @return Number of bytes sent, or -1 on error
     */
    ssize_t sendto(const void* data, size_t size, const NetworkEndpoint& endpoint);

    /**
     * @brief Receive datagram with sender information
     * @param data Buffer to receive into
     * @param size Maximum size to receive
     * @param sender Output parameter for sender endpoint
     * @return Number of bytes received, or -1 on error
     */
    ssize_t recvfrom(void* data, size_t size, NetworkEndpoint& sender);
